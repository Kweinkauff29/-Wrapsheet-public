// WrapSheet Public - Multi-Tenant SaaS Version
// Based on BER Wrap Sheet Worker v4

const CORS_HEADERS = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, PATCH, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Accept, x-user-email, x-org-id, x-org-slug",
};
...
// Get org_id from request header or query param
async function getOrgId(request, env, searchParams) {
    // Check header first
    ...
    // Check for org slug header
    const orgSlugHeader = request.headers.get("x-org-slug");
    if (orgSlugHeader) {
        const org = await env.WRAP_DB
    ...
    if (searchParams) {
        ...
        if (orgIdParam) return Number(orgIdParam);
...
async function listOrganizations(env) {
    const { results } = await env.WRAP_DB
        .prepare(`SELECT id, name, slug, logo_url, primary_color, secondary_color, created_at FROM organizations ORDER BY name`)
        .all();
    return json(results);
}

async function getOrganization(env, identifier) {
    // Try by slug first, then by ID
    let org = await env.WRAP_DB
...
async function createOrganization(request, env) {
    ...
    const { name, slug, logo_url = null, primary_color = '#03aae1', secondary_color = '#fbbf24', settings = null } = body;
    ...
    try {
        const info = await env.WRAP_DB
            .prepare(`
                INSERT INTO organizations (name, slug, logo_url, primary_color, secondary_color, settings, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `)
            .bind(name, slug, logo_url, primary_color, secondary_color, settings ? JSON.stringify(settings) : null, now, now)
            .run();
        ...
        if (err.message?.includes("UNIQUE constraint")) {
            return json({ error: "Organization slug already exists" }, 409);
        }
        throw err;
    }
}
...
async function updateOrganization(request, env, id) {
    ...
    const logo_url = body.logo_url !== undefined ? body.logo_url : existing.logo_url;
    const primary_color = body.primary_color ?? existing.primary_color;
    const secondary_color = body.secondary_color ?? existing.secondary_color;
    const settings = body.settings !== undefined ? JSON.stringify(body.settings) : existing.settings;
    const now = new Date().toISOString();

    await env.WRAP_DB
        .prepare(`
            UPDATE organizations
            SET name = ?, logo_url = ?, primary_color = ?, secondary_color = ?, settings = ?, updated_at = ?
            WHERE id = ?
        `)
        .bind(name, logo_url, primary_color, secondary_color, settings, now, id)
        .run();

    const updated = await env.WRAP_DB
        .prepare(`SELECT * FROM organizations WHERE id = ?`)
        .bind(id)
        .first();
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            const orgId = await getOrgId(request, env, searchParams);

            // ===== ORGANIZATIONS =====
            if (pathname === "/api/organizations" && request.method === "GET") {
                return await listOrganizations(env);
            }
            ...
            if (pathname.startsWith("/api/organizations/")) {
                ...
                if (request.method === "GET") {
                    return await getOrganization(env, identifier);
                }
            ...
            if (pathname === "/api/users" && request.method === "GET") {
                return await listUsers(env, searchParams, orgId);
            }
